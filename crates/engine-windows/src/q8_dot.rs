...
/// The numeric contract [`q8_0_dot_rows`] holds to, declared beside the kernel
/// it describes so an edit that weakens the kernel cannot leave the claim
/// behind. The serving binary publishes it on every response.
///
/// What makes it true is not this line. It is
/// `dispatched_dot_matches_the_portable_reference_on_random_blocks` over the
/// full i8 domain including `-128`, `avx2_dot_is_bit_identical_to_the_portable_reference`
/// over row lengths 0..=33 and both mismatched directions,
/// `an_empty_reduction_keeps_the_reference_sign_of_zero` for the `-0.0` seed the
/// module docs single out, and `degenerate_scale_band_matches_byte_for_byte` for
/// the quantizer band engine-macos cannot close. A constant that outlived those
/// tests would be a claim with nothing under it.
///
/// What it does **not** claim, since both are easy to read into it:
///
/// The AVX2-versus-portable dispatch inside [`q8_0_dot_rows`] selects a path,
/// not a posture. Both arms produce the same bits — the fallback *is* the
/// reference — which is why a host without AVX2 publishes this same value rather
/// than a weaker one.
///
/// And the claim is over this kernel, not over the replica's other arithmetic.
/// Everything else in the forward pass is engine-core's portable code on both
/// sides of the seam; this says the seam does not move it.
...
mod tests {
    ...
    /// The published token, pinned. The substantive claim is carried by the
    /// bit-identity tests named on [`POSTURE`]; this pins the string those tests
    /// entitle the replica to put on the wire, which is a published value with
    /// the same retirement cost as a digest and is in no digest preimage.
    #[test]
