...
//! binary passes to engine-core through its Q8_0 projection seam. The Linux port
//! is
...
//!
//! That output-invariance is why the replica's published execution posture stays
//! `cpu` on both arms: the posture names which forward-pass implementation ran,
//! and the AVX2 arm is not a second implementation of the forward pass — it is
//! the same one with a faster leaf, proven to the bit. A backend that changed
//! the numbers would be the case that posture exists to distinguish, and it is
//! not this one.
//!
