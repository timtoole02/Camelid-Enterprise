...
//! because it is the general acceleration contract. A backend admitted under a
//! published tolerance would publish a different execution posture and need a
//! different gate; this one would correctly refuse it rather than quietly widen,
//! which is the right failure for a file whose whole claim is "to the bit".
//!
...
/// The control as a projection, so it enters the forward pass through the same
/// seam the real kernel does.
///
/// Built on `project_rows` for the same reason the real projection is: driving
/// the control through a different row-splitting path than the kernel it is
/// controlling for would leave the comparison measuring two things at once.
fn one_ulp_perturbed_projection(
...
fn avx2_accelerated_forward_is_bit_identical_in_the_logits() {
    ...
    let mut accelerated =
        Decoder::with_q8_projection(&config, &weights, engine_windows::q8_0_project_rows).unwrap();
    ...
    let mut control = Decoder::with_q8_projection(&config, &weights, one_ulp_perturbed_projection).unwrap();
