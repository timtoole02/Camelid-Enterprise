...
/// Builds the forwarding surface from [`replica_contract::PUBLIC_ROUTES`].
///
/// The gateway used to declare this as a hand-written axum route table mirroring
/// the same ten routes, with a comment conceding that nothing verified the two
/// agreed. That was a second inventory of one public surface, and the drift it
/// admitted was silent and one-directional: a route added to the contract and
/// served by the replica would have been answered `404` here, at the component
/// the deployment documents as the trust boundary — an availability failure
/// visible only in production, on exactly the release that added the route.
///
/// There is one list now and both processes read it. `replica-contract` is
/// dependency-free for this reason: the gateway links the public surface without
/// linking the engine, so nothing about taking the dependency drags the
/// inference stack into a proxy.
///
/// What this does **not** do is turn the gateway into a second enforcement
/// point for the replica's rules. The registry says which routes are public; the
/// replica still decides what each one answers, and the generation-body check
/// stays there because only the replica knows which weights it hashed.
fn forwarded_routes() -> Router<GatewayState> {
    let mut router = Router::new();
    for route in replica_contract::PUBLIC_ROUTES {
        let mut methods = MethodRouter::new();
        for method in route.methods {
            // Exhaustive on purpose: a method the registry grows later stops the
            // build here rather than being silently dropped from the gateway's
            // surface by a catch-all arm.
            methods = match method {
                HttpMethod::Get => methods.get(proxy),
                HttpMethod::Post => methods.post(proxy),
                HttpMethod::Delete => methods.delete(proxy),
            };
        }
        router = router.route(route.path, methods);
    }
    router
}
...
pub fn router_with_options(
    ...
    forwarded_routes()
        // Gateway-local, and deliberately not in the registry: it reports on the
        // gateway process, never reaches a replica, and is not part of the
        // client-facing inference contract.
