...
/// instead is bit-identical everywhere else and wrong there. engine-macos's
/// kernels seed `-0.0` for exactly this reason; pinned by
...
mod tests {
    ...
    /// wrong while passing every other bit-identity check — engine-macos did,
    /// until it was corrected to seed `-0.0`.
    #[test]
