...
**Parity-oracle revision:** `b4e3a9056567ed8145fc4fa29850d6f1f261ac2b`
...
route present in the historical parity oracle is a stable Enterprise API.
...
replicas and gateways share it without linking an inference engine. Both do:
...
between releases. The in-tree implementation lives in
`crates/server/src/in_tree.rs`. A private pinned-route inventory remains in
`crates/server/src/contract.rs` only as a test oracle for the migration baseline.
...
- **Executable (no model):** exercised directly against the in-tree router and
  its production surface filters. Separate legacy tests drive the pinned oracle
  to preserve exact compatibility messages and the migration baseline.
- **Executable (model):** exercised by the ignored model-backed conformance test
  ...
  and not about a router nothing serves. The model-backed workflow runs it with
  the pinned GGUF artifact.
- **Pinned source:** verified against the exact oracle revision above, but no
...
compatibility, workspace, model-lifecycle, and embedded WebUI route is outside
the public contract. The in-tree production router does not implement those
surfaces, and the outer route filter refuses them uniformly.
...
Axum also answers `HEAD` for registered `GET` routes. The in-tree router provides
...
This is not a complete required/optional schema declaration. The in-tree server
owns field validation and fails closed on request features it has not explicitly
implemented. Promote more field-level semantics here only with executable
conformance coverage and a deliberate contract review.

The deterministic output claim applies to greedy requests (`temperature: 0`)
within one in-tree build, frozen configuration vector, hardware class, thread
...
`GET /v1/health` reports process health and generation readiness for the one
model loaded before the replica begins serving:
...
`GET /v1/models` contains exactly the one startup model. Production does not
serve a model-less router and exposes no API that can add or remove models.

A missing `GET /v1/models/{model}` returns HTTP `404` with error code
`model_not_found`.

**Evidence:** focused in-tree tests and the model-backed production conformance
test.

## Completion responses

Non-streaming completion response schemas are owned by the in-tree server and
follow the OpenAI-compatible shapes pinned by the model-backed contract.
Contract v1 additionally guarantees the Enterprise attribution fields above.
...
**Evidence:** in-tree source plus the model-backed conformance workflow. The
default cross-platform matrix compiles this test but does not download weights.
...
The model is loaded directly into the in-tree runtime before the router begins
serving. A load failure exits the process instead of serving a model-less
replica, and there is no HTTP load/unload control plane.
...
**Evidence:** production composition source, lane tests, and the model-backed
replica contract. Process-exit behavior remains an operator-level check.

## Parity-oracle private implementation inventory
...
methods at the pinned oracle revision — it catches a declared route being
...
These categories exist in the pinned oracle but are absent from the production
router and are **not** Enterprise public contracts:
