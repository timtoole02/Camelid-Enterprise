...
| **Platform kernels** | `crates/engine-{macos,linux,windows}` | Runtime CPU feature detection (`probe()`), platform kernels. macOS supplies a NEON/SDOT Q8_0 projection through engine-core's kernel seam and Windows an AVX2 one, each tested bit-identical to the portable reference; Linux is currently capability-detection only. |
