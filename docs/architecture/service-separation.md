...
| **Attribution middleware** | `crates/server/src/attribution.rs` | Stamps seven headers on every response — `x-camelid-lane`, `-config-sha256`, `-admission-sha256`, `-model-sha256`, `-host`, `-worker-threads`, `-posture` — injects the same facts into completion bodies, and writes optional JSONL serving receipts with the digests at full length, each carrying the gateway-stamped `request_id` when present (`null` for direct-to-replica traffic). |
...
| **Platform kernels** | `crates/engine-{macos,linux,windows}` | Runtime CPU feature detection (`probe()`), platform kernels. macOS supplies a NEON/SDOT Q8_0 dot through engine-core's kernel seam and Windows an AVX2 one, each tested bit-identical to the portable reference; Linux is currently capability-detection only. |
...
- **Opt-in local identity.** SQLite-backed users, organizations, memberships,
  ...
  federation layer. The replica itself has no caller identity or authorization;
  identity remains at the gateway.
