...
| **Transparent gateway** | `crates/gateway` (`camelid-enterprise-gateway` bin) | Fixed-origin forwarding for an explicit `/v1` inference allowlist, with opaque streaming bodies, hop-by-hop header filtering, no retries, bounded concurrency, and no response rewriting. Replica control routes are not exposed. |
...
1. **Baseline & contracts — implemented.**
  ...
  checked against the exact pinned router without invoking handlers; no-model
  tests cover health, discovery, typed errors, and attribution. An explicit
  model-backed test covers load, readiness, deterministic greedy output, and
  SSE with a compatible local GGUF. Queue saturation remains source-verified
  rather than independently stress-tested.
